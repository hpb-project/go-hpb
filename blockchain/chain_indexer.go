...
type ChainIndexerChain interface {
	...
	SubscribeChainHeadEvent(ch chan<- ChainHeadEvent) sub.Subscription
}
...
func NewChainIndexer(chainDb hpbdb.Database, indexDb hpbdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
...
// queue.
func (c *ChainIndexer) eventLoop(currentHeader *types.Header, events chan ChainHeadEvent, sub sub.Subscription) {
	...
	for {
		...
		case ev, ok := <-events:
			...
			if header.ParentHash != prevHash {
				...
				if GetCanonicalHash(c.chainDb, prevHeader.Number.Uint64()) != prevHash {
	...
	if head >= c.confirmsReq {
		...
		if sections > c.knownSections {
			if c.knownSections < c.checkpointSections {
				...
				syncedHead := GetCanonicalHash(c.chainDb, c.checkpointSections*c.sectionSize-1)
