...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == NewBlockHashesMsg:
	    ...
	    log.Info("#################handler->handleMsg->NewBlockHashesMsg")
	...
	case msg.Code == NewBlockMsg:
		...
		log.Info("##########################handler->handleMsg->NewBlockMsg")
		...
		if _, td := p.Head(); trueTD.Cmp(td) > 0 {
			...
			if trueTD.Cmp(pm.blockchain.GetTd(currentBlock.Hash(), currentBlock.NumberU64())) > 0 {
				log.Info("@@@@@@@@@@@@handler->pm.synchronise")
				go pm.synchronise(p)
			}
		}
...
func (pm *ProtocolManager) BroadcastBlock(block *types.Block, propagate bool) {
	...
	if propagate {
		...
		log.Info("#######################handler->BroadcastBlock-->propagate")
	...
	if pm.blockchain.HasBlock(hash, block.NumberU64()) {
		...
		log.Info("#######################handler->BroadcastBlock-->pm.blockchain.HasBlock")
...
func (self *ProtocolManager) minedBroadcastLoop() {
	...
	for obj := range self.minedBlockSub.Chan() {
		...
		case core.NewMinedBlockEvent:
		    log.Info("#######################handler->minedBroadcastLoop")
