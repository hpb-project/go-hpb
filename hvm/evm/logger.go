...
func (l *StructLogger) CaptureState(env *EVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory, stack *Stack, rStack *ReturnStack, rData []byte, contract *Contract, depth int, err error) error {
	...
	if !l.cfg.DisableStorage {
		...
		if op == SLOAD && stack.len() >= 1 {
			var (
				address = common.Hash(uint256.NewInt().SetUint64(stack.data[stack.len()-1].Uint64()).Bytes32())
		...
		if op == SSTORE && stack.len() >= 2 {
			var (
				value   = common.Hash(uint256.NewInt().SetUint64(stack.data[stack.len()-2].Uint64()).Bytes32())
				address = common.Hash(uint256.NewInt().SetUint64(stack.data[stack.len()-1].Uint64()).Bytes32())
			)
			l.storage[contract.Address()][address] = value
		}
	...
	log := StructLog{pc, op, gas, cost, mem, memory.Len(), stck, rstack, rdata, storage, depth, env.StateDB.GetRefund().Uint64(), err}
