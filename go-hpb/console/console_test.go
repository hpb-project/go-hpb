...
func newTester(t *testing.T, confOverride func(*hpb.Config)) *tester {
	...
	ethConf := &hpb.Config{
		...
		Hpberbase: common.HexToAddress(testAddress),
		PowTest:   true,
	}
	if confOverride != nil {
		confOverride(ethConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return hpb.New(ctx,
		ethConf) }); err != nil {
