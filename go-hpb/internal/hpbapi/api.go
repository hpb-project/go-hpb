...
// PublicHpbereumAPI provides an API to access Ethereum related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicHpbereumAPI struct {
	b Backend
}

// NewPublicHpbereumAPI creates a new Ethereum protocol API.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", hpbhash.SeedHash(number)), nil
}
