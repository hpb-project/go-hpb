...
func newCanonical(n int) (hpbdb.Database, *LightChain, error) {
	...
	blockchain, _ := NewLightChain(&dummyOdr{db: db}, gspec.Config, hpbhash.NewFaker())
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, hpbhash.NewFullFaker())
	if err != nil {
		panic(err)
	}
	return lc
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, hpbhash.NewFaker())
	if err != nil {
