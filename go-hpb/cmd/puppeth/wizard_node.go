...
func (w *wizard) deployNode(boot bool) {
	...
	if infos.hpbstats == "" {
		...
		infos.hpbstats = w.readString() + ":" + w.conf.ethstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.hpbstats)
		infos.hpbstats = w.readDefaultString(infos.hpbstats) + ":" + w.conf.ethstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.genesis.Config.Hpbhash != nil {
			// Hpbhash based miners only need an etherbase to mine against
			fmt.Println()
			if infos.hpberbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.hpberbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.hpberbase)
				infos.hpberbase = w.readDefaultAddress(common.HexToAddress(infos.hpberbase)).Hex()
			}
