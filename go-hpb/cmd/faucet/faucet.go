...
var (
	...
	hpbPortFlag = flag.Int("hpbport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Hpbereum protocol")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for Hpbereum and the faucet")
)
...
func main() {
	...
	faucet, err := newFaucet(genesis, *hpbPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // Hpbereum address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Hpbereum light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Hpbereum protocol stack
	client *hpbclient.Client   // Client connection to the Hpbereum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Hpbereum light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := hpb.DefaultConfig
	...
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightHpbereum
			ctx.Service(&serv)
			return hpbstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := hpbclient.NewClient(api)
...
// close terminates the Hpbereum connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Iterate over all the files and look for Hpbereum addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No Hpbereum address found to fund"})
			continue
		}
