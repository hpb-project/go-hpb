...
import (
	...
	"github.com/hpb-project/go-hpb/hpbstats"
	"github.com/hpb-project/go-hpb/les"
...
var (
	...
	// Hpbhash settings
	HpbhashCacheDirFlag = DirectoryFlag{
		Name:  "hpbhash.cachedir",
		Usage: "Directory to store the hpbhash verification caches (default = inside the datadir)",
	}
	HpbhashCachesInMemoryFlag = cli.IntFlag{
		Name:  "hpbhash.cachesinmem",
		Usage: "Number of recent hpbhash caches to keep in memory (16MB each)",
		Value: hpb.DefaultConfig.HpbhashCachesInMem,
	}
	HpbhashCachesOnDiskFlag = cli.IntFlag{
		Name:  "hpbhash.cachesondisk",
		Usage: "Number of recent hpbhash caches to keep on disk (16MB each)",
		Value: hpb.DefaultConfig.HpbhashCachesOnDisk,
	}
	HpbhashDatasetDirFlag = DirectoryFlag{
		Name:  "hpbhash.dagdir",
		Usage: "Directory to store the hpbhash mining DAGs (default = inside home folder)",
		Value: DirectoryString{hpb.DefaultConfig.HpbhashDatasetDir},
	}
	HpbhashDatasetsInMemoryFlag = cli.IntFlag{
		Name:  "hpbhash.dagsinmem",
		Usage: "Number of recent hpbhash mining DAGs to keep in memory (1+GB each)",
		Value: hpb.DefaultConfig.HpbhashDatasetsInMem,
	}
	HpbhashDatasetsOnDiskFlag = cli.IntFlag{
		Name:  "hpbhash.dagsondisk",
		Usage: "Number of recent hpbhash mining DAGs to keep on disk (1+GB each)",
		Value: hpb.DefaultConfig.HpbhashDatasetsOnDisk,
	}
...
func setHpbhash(ctx *cli.Context, cfg *hpb.Config) {
	if ctx.GlobalIsSet(HpbhashCacheDirFlag.Name) {
		cfg.HpbhashCacheDir = ctx.GlobalString(HpbhashCacheDirFlag.Name)
	}
	if ctx.GlobalIsSet(HpbhashDatasetDirFlag.Name) {
		cfg.HpbhashDatasetDir = ctx.GlobalString(HpbhashDatasetDirFlag.Name)
	}
	if ctx.GlobalIsSet(HpbhashCachesInMemoryFlag.Name) {
		cfg.HpbhashCachesInMem = ctx.GlobalInt(HpbhashCachesInMemoryFlag.Name)
	}
	if ctx.GlobalIsSet(HpbhashCachesOnDiskFlag.Name) {
		cfg.HpbhashCachesOnDisk = ctx.GlobalInt(HpbhashCachesOnDiskFlag.Name)
	}
	if ctx.GlobalIsSet(HpbhashDatasetsInMemoryFlag.Name) {
		cfg.HpbhashDatasetsInMem = ctx.GlobalInt(HpbhashDatasetsInMemoryFlag.Name)
	}
	if ctx.GlobalIsSet(HpbhashDatasetsOnDiskFlag.Name) {
		cfg.HpbhashDatasetsOnDisk = ctx.GlobalInt(HpbhashDatasetsOnDiskFlag.Name)
	}
}
...
// RegisterEthStatsService configures the Hpbereum Stats daemon and adds it to
// th egiven node.
