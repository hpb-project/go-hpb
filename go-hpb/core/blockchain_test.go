...
func newTestBlockChain(fake bool) *BlockChain {
	...
	gspec.MustCommit(db)
	engine := hpbhash.NewFullFaker()
	if !fake {
		engine = hpbhash.NewTester()
	}
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	ncm, err := NewBlockChain(bc.chainDb, bc.config, hpbhash.NewFaker(), vm.Config{})
	if err != nil {
...
func testInsertNonceError(t *testing.T, full bool) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		if full {
			...
			blockchain.engine = hpbhash.NewFakeFailer(failNum)
...
func TestFastVsFullChains(t *testing.T) {
	...
	archive, _ := NewBlockChain(archiveDb, gspec.Config, hpbhash.NewFaker(), vm.Config{})
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, gspec.Config, hpbhash.NewFaker(), vm.Config{})
	defer fast.Stop()
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	light, _ := NewBlockChain(lightDb, gspec.Config, hpbhash.NewFaker(), vm.Config{})
...
func TestChainTxReorgs(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(db, gspec.Config, hpbhash.NewFaker(), vm.Config{})
