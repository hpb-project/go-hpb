...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proBc, _ := NewBlockChain(proDb, &proConf, hpbhash.NewFaker(), vm.Config{})
	defer proBc.Stop()

	conDb, _ := hpbdb.NewMemDatabase()
	...
	conBc, _ := NewBlockChain(conDb, &conConf, hpbhash.NewFaker(), vm.Config{})
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		db, _ = hpbdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ := NewBlockChain(db, &conConf, hpbhash.NewFaker(), vm.Config{})
		defer bc.Stop()
		...
		db, _ = hpbdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ = NewBlockChain(db, &proConf, hpbhash.NewFaker(), vm.Config{})
		defer bc.Stop()
