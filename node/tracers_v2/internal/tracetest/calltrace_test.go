...
import (
	...
	_ "github.com/hpb-project/go-hpb/node/tracers_v2/js"
	_ "github.com/hpb-project/go-hpb/node/tracers_v2/native"
)
...
func testCallTracer(tracerName string, dirPath string, t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(file.Name(), ".json")), func(t *testing.T) {
			...
			st := core.NewStateTransition(evm, msg, new(core.GasPool).AddGas(tx.Gas().Uint64()), &types.Header{Number: new(big.Int).SetUint64(uint64(test.Context.GasLimit))})
...
func TestZeroValueToNotExitCall(t *testing.T) {
	...
	tx, err := types.SignTx(unsignedTx, signer, privkey)
	if err != nil {
		t.Fatalf("err %v", err)
	}
	origin, _ := signer.Sender(tx)
