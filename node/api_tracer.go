		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					var res interface{}
					if hpbconfig.UseNewEvm(task.block.NumberU64()) {
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if hpbconfig.UseNewEvm(uint64(number)) {
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if hpbconfig.UseNewEvm(block.Number().Uint64()) {
		return api.traceBlock_evm(ctx, block, config)
	}
	return api.traceBlock(ctx, block, config)
}
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	if hpbconfig.UseNewEvm(block.NumberU64()) {
...
func (api *PrivateDebugAPI) TraceCall(ctx context.Context, args hpbapi.CallArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceConfig) (interface{}, error) {
	...
	if hpbconfig.UseNewEvm(header.Number.Uint64()) {
