...
import (
	...
	config "github.com/hpb-project/go-hpb/config"
...
func runTrace(tracer *JavascriptTracer) (interface{}, error) {
	env := vm.NewEVM(vm.Context{BlockNumber: big.NewInt(0)}, nil, config.MainnetChainConfig, vm.Config{Debug: true, Tracer: tracer})
