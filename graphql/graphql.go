...
func (t *Transaction) Gas(ctx context.Context) (hexutil.Uint64, error) {
	...
	return hexutil.Uint64(tx.Gas().Uint64()), nil
}
...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, err := hpbapi.DoCall(ctx, b.backend, args.Data, *b.numberOrHash, nil, vm.Config{}, 5*time.Second, b.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, err := hpbapi.DoCall(ctx, p.backend, args.Data, pendingBlockNr, nil, vm.Config{}, 5*time.Second, p.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
